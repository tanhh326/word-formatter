...
public class SymbolAbbreviationsFormatter extends AbstractFormatter {
  ...
  public SymbolAbbreviationsFormatter(FormattingProcessShareVar formattingProcessShareVar) {
    super(formattingProcessShareVar);
  }

  @Override
  public SectionEnums sectionEnums() {
    return SectionEnums.符号和缩略语说明;
  }
  ...
  public void confirmTitle(Paragraph paragraph) throws Exception {
    ...
    if (titles.isEmpty() &&
        ...
        MatchUtils.match(text, "符号和缩略语说明")
    ) {
  ...
  public void formatTitle() {
    if (titles.isEmpty()) {
      ...
      title.getRuns().add(new Run(studetDocument, "符号和缩略语说明"));
      titles.add(title);
    }
