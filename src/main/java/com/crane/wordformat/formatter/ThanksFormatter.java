...
public class ThanksFormatter extends AbstractFormatter {

  private final StyleConfigDto styleConfigDto = formattingProcessShareVar.getStyleConfig(
      sectionEnums());


  public ThanksFormatter(FormattingProcessShareVar formattingProcessShareVar) {
    super(formattingProcessShareVar);
  }

  @Override
  public SectionEnums sectionEnums() {
    return SectionEnums.致谢;
  }
  ...
  public void formatTitle() {
    if (titles.isEmpty()) {
      ...
      title.getRuns().add(new Run(studetDocument, "致谢"));
      titles.add(title);
    }
